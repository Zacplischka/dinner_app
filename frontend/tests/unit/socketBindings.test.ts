// socketBindings owns the socket transport (connection lifecycle, ack-based
// commands) and every socket → UI wiring: store mutations and toasts.
...
describe('socketBindings', () => {
  ...
  // #518: straight to WebSocket — no serial long-polling round trips before
  // connect. A finite reconnection cap left the Manager dead after ~15s of
  // backoff while the header still read "Reconnecting..."; the Session's own
  // TTL is the bound.
  it('connects over WebSocket only, with no handshake auth and no reconnection cap', () => {
    setupSocket();
    socketBindings.initializeSocket();
  ...
  it('never orphans a socket that is mid-reconnect', () => {
    ...
    socketBindings.initializeSocket();
    void socketBindings.waitForConnection(50).catch(() => undefined);
  ...
  it('waits for a connection, and rejects on a connection error or timeout', async () => {
    vi.useFakeTimers();
    const socket = setupSocket(false);
    socketBindings.initializeSocket();

    const connected = expect(socketBindings.waitForConnection()).resolves.toBeUndefined();
    socket.trigger('connect');
    socket.connected = true;
    await connected;
    await expect(socketBindings.waitForConnection()).resolves.toBeUndefined();

    socket.connected = false;
    const failed = expect(socketBindings.waitForConnection()).rejects.toThrow('connect failed');
    socket.trigger('connect_error', new Error('connect failed'));
    await failed;

    const timedOut = expect(socketBindings.waitForConnection(10)).rejects.toThrow(
  ...
  it('returns a failure Ack for every command while disconnected', async () => {
    setupSocket(false);
    socketBindings.initializeSocket();
    ...
    await expect(socketBindings.joinSession('AB123', 'Alice')).resolves.toEqual(notConnected);
    await expect(socketBindings.submitSelection('AB123', [])).resolves.toEqual(notConnected);
    await expect(socketBindings.restartSession({ sessionCode: 'AB123' })).resolves.toEqual(
      notConnected
    );
    await expect(socketBindings.leaveSession('AB123')).resolves.toEqual(notConnected);
  });
  ...
  it('fails a command whose ack never arrives instead of hanging forever', async () => {
    ...
    const settled = vi.fn();
    void socketBindings.submitSelection('AB123', ['place-1']).then(settled);
  ...
  it('does not start recovery after a lost basket read', async () => {
    vi.useFakeTimers();
    const socket = setupSocket();
    socketBindings.initializeSocket();
    // Enough for recovery to run, which would first drop isConnected.
    useSessionStore.setState({
      sessionCode: 'AB123',
      currentUserId: participant.participantId,
      isConnected: true,
    ...
    const read = socketBindings.openOrder({ sessionCode: 'AB123', placeId: 'place-1' });
  ...
  it('blocks a manual retry after a lost Order Line ack until the basket is reconciled', async () => {
    ...
    const first = socketBindings.addOrderItem({ sessionCode: 'AB123', index: 0, delta: 1 });
    await vi.advanceTimersByTimeAsync(10_000);
    ...
    expect
      .soft(await socketBindings.addOrderItem({ sessionCode: 'AB123', index: 0, delta: 1 }))
      .toMatchObject({
        success: false,
        error: {
    ...
    const retriedRecovery = socketBindings.reconcileSession();
    ...
    expect.soft(itemEmits()).toHaveLength(1);
    expect(
      await socketBindings.addOrderItem({ sessionCode: 'AB123', index: 0, delta: 1 })
    ).toMatchObject({ success: true });
    expect.soft(itemEmits()).toHaveLength(2);

    readBasket = undefined;
    const recoveryBeforeRestart = socketBindings.reconcileSession();
    await vi.advanceTimersByTimeAsync(0);
    expect(readBasket).toBeDefined();
    socket.trigger('session:restarted', {
      sessionCode: 'AB123',
  ...
  // Last: resetModules leaves later dynamic imports on fresh module copies.
  it('connects to the configured backend URL', async () => {
    vi.resetModules();
    vi.stubEnv('VITE_BACKEND_URL', 'https://socket.example.test');
    const fresh = await import('../../src/services/socketBindings');
    setupSocket();

    fresh.initializeSocket();
