...
export default function FriendsList({
  ...
  // ponytail: one failure at a time — the confirm dialog serialises removes, so
  // a second attempt can only start once the first has reported back.
  ...
  const handleRemoveFriend = async (friendId: string, displayName: string) => {
    ...
    setRemoveError(null);
    if (!(await removeFriend(friendId))) {
      setRemoveError({
        id: friendId,
        message: useFriendsStore.getState().error ?? `Failed to remove ${displayName}`,
      });
    }
  };
