// socketBindings - the Socket.IO client and its UI wiring: connection
// lifecycle, ack-based commands, and every store mutation and toast triggered
// by socket traffic. Pages import from here.
...
function mutationBlocked(event: keyof ClientToServerEvents): string | undefined {
...
// It resolves as-is; the only acks the client mints itself are the
// not-connected and timed-out failures below. Without the timeout a
...
function emitAck<T>(event: keyof ClientToServerEvents, payload: unknown): Promise<Ack<T>> {
  return new Promise((resolve) => {
    ...
    (
      ...
      if (err && recovers) void reconcileSession();
      resolve(
        err
          ? {
              success: false,
...
/**
 * Initialize the Socket.IO connection with the app's UI wiring.
 */
...
/**
 * Wait for socket to be connected, initializing it if needed.
 * Resolves when connected, rejects on a connection error or timeout.
 */
...
async function admitSession(
  ...
  const stale = () => !isSessionIntentCurrent(generation) || (resumeOnly && !stillResuming());
  // Finish departure before a queued newer join, even if the older ack timed out.
  const abandon = async (ack: Ack<SessionJoinData>) => {
    if (!resumeOnly) {
      if (ack.success && intendedParticipant === `${sessionCode}:${displayName}`) {
        ...
        await emitAck<null>('session:leave', { sessionCode });
  ...
  const ack = await emitAck<SessionJoinData>('session:join', {
    sessionCode,
    displayName,
    rejoinToken: token ?? undefined,
    accessToken: useAuthStore.getState().session?.access_token,
  } satisfies SessionJoinPayload);
  if (stale()) return abandon(ack);
  if (!ack.success) {
    if (
...
/**
 * Commands that touch no store and raise no toast: each one's effect reaches
 * the UI as a server broadcast, not a local write. addOrderItem and claimBuyer
 * are the clearest case — the order:state broadcast includes the sender, so it
 * is what updates every basket, this phone's too.
 */
export const restartSession = (p: SessionRestartPayload) => emitAck<null>('session:restart', p);
export const updateSessionChoices = (p: SessionChoicesPayload) =>
  emitAck<SessionLobbyState>('session:choices', p);
export const setSessionReady = (p: SessionReadyPayload) =>
  emitAck<SessionLobbyState>('session:ready', p);
export const startSession = (p: SessionLobbyPayload) =>
  emitAck<SessionLobbyState>('session:start', p);
export const removeSessionParticipant = (p: SessionRemovePayload) =>
  emitAck<SessionLobbyState>('session:remove', p);
...
// "I'll order": first tap wins. claimBuyer doubles as the Buyer's debounced
...
export function sendLiveSelection(
  ...
  const lobby = useSessionStore.getState().lobby;
  return emitAck<null>('selection:live', {
    sessionCode,
    placeId,
    retract,
    round: lobby?.sessionCode === sessionCode ? lobby.round : undefined,
  } satisfies SelectionLivePayload);
}
...
export async function leaveSession(
  ...
  return queueAdmission(async () => {
    if (!isSessionIntentCurrent(generation)) return superseded();
    try {
      const ack = await emitAck<null>('session:leave', { sessionCode });
...
export function reconcileSession(): Promise<void> {
  ...
  recovery = (async () => {
    try {
      ...
      if (orderPlaceId) {
        const order = await openOrder({ sessionCode: code, placeId: orderPlaceId });
        if (!stillCurrent()) return;
        const current = useSessionStore.getState();
