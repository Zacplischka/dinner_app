...
Signed archive/AAB automation, signing recovery, build-number and artifact/symbol provenance remain outstanding. Native compile CI has been added as described below; both hosted compile jobs passed on commit `76f4ca2`. No command in this document submits to a store.
...
| Android | `ubuntu-24.04`, Temurin **21.0.12+8.0.LTS**, SDK **36**, Build-Tools **35.0.0** | Checked-in Gradle wrapper runs `:app:assembleDebug :app:assembleDebugAndroidTest`.                                        |
...
The workflow definition passed local YAML, shell syntax, manifest/path and formatting checks. Both platforms then passed clean hosted compilation on commit `76f4ca2` in [run 34194507636](https://github.com/Zacplischka/dinner_app/actions/runs/34194507636). The first hosted attempts exposed a Temurin build-identifier mismatch and the existing web workflow's obsolete Node 20 runtime; CI and the root runtime declaration now use Node 22. Account for path filters when configuring required checks. Signed distribution, controlled release credentials, crash symbols and artifact provenance remain separate work.
