...
- Uber Eats search + menus: `borderline/uber-eats-scraper-ppr` — address + `addressCountry: AU` + `storeType: RESTAURANTS`; full menus, prices in cents AUD, item-level promo tags
- DoorDash menu: `crawlerbros/doordash-restaurant-scraper` — store URL in, full menu out (`A$` prices)
- DoorDash store details: `tri_angle/doordash-store-details-scraper` — store URL in, metadata + lat/lng out (used for geo-verifying the name match)
...
- Observed run times: ~1–3 min per actor. HTTP client timeouts ≥300s; run the two platform chains in parallel.
- Cost discipline: actors are pay-per-result. Keep `maxRows`/`maxItems` minimal (≤5), and never call Apify outside `ComparisonService` — one choke point for rate limiting and tests. Free credit is $5/month: fine for dev + personal prod use, not for load testing.

### Payload parsing facts (verified against the captured fixtures)

- UE menu: `menu[]` sections → `catalogItems[]`; `price` is an **integer in AUD cents** (785 = $7.85); deals are per-item `tags` strings ("30% off"); store-level `url` is the outbound link; `currencyCode: "AUD"` confirms market.
...
- DD menu (`crawlerbros`): flat `menuItems[]` with `section`, `name`, `price` as a **string** `"A$23.00"` — parse to cents. No deals anywhere in DD payloads (render the Deals cell as "—", not an error).
- DD details (`tri_angle`): `lat`/`lng` floats for the geo check, `currency: "AUD"`, `numRatings`/`rating`.
...
**DoorDash** (recipe — validate before building, see build order step 0): store URLs embed an unguessable ID (`.../store/melbourne-pizza-factory-melbourne-843606/`) and no actor searches DD by location (see the fixtures `AGENTS.md` gotcha). In order:

1. Newest Snapshot for the placeId has a DD `storeUrl` → use it.
2. Otherwise query the **Google Custom Search JSON API** (free 100 queries/day — plenty) with `site:doordash.com/store "<venue name>" <suburb>`; fetch **all candidate store URLs in a single `tri_angle` run** (`startUrls` is already an array — one actor run regardless of candidate count), accept on the same geo+name check. This automates the manual search that found the sample store. Needs a Programmable Search Engine id + API key — gcloud is authenticated locally but **verify/switch the active project first** (defaults to an unrelated one; see CLAUDE.md).
3. No acceptable hit → `status: not_found` for DoorDash; the UX renders that honestly.

If CSE proves unreliable at step 0, surface that finding rather than forcing it — a UE-only view is a fallback **product decision** requiring renegotiation of AC2 and the DoD (both assume two Platforms), not a quiet degradation.

## Build order

0. **Validate the DoorDash discovery recipe by hand for ~3 known venues** (see Storefront resolution recipes) before building on it. While there: re-capture a UE sample using a **venue-name** query (the existing sample used a cuisine query — the resolution path has no fixture), capture a DD menu sample from a CSE-resolved URL, and **test the UE actor's `urls` input mode** with the sample's store URL (schema supports it; if it works, stale re-Compares skip the UE search too — see AC4); add captures to `backend/tests/fixtures/comparison/` with AGENTS.md entries
1. Venue list endpoint: reuse `RestaurantSearchService` (Google Places) for nearby Venues — no actors involved
2. Backend `ComparisonService` + Apify client: the compare flow starts with a **server-side Google Place Details lookup by placeId** (name + coords — the client never supplies them, which also gives cold-opened URLs a real venue name immediately) → per-Venue resolution per the resolution recipes → menu + details actors → ~100m geo verify → insert Snapshot (Supabase migration for `comparison_snapshots` comes first in this step). Shape: `createComparisonService(deps)` with plain function deps — `runActor(actorId, input)`, `cseSearch`, `fetchPlaceDetails`, `snapshotStore`, plus `freshnessMs`/`settleCapMs` constants — same DI pattern as `createFriendsService(deps)`; tests pass inline fakes
3. Item matching: **v1 normalization is deliberately dumb** — lowercase, strip punctuation, collapse whitespace, exact match on the result; no fuzzy matching until real menus prove the need (unmatched items have a home in the UI by design). "Menu ~X% cheaper" = **median** per-item % difference over matched pairs (median resists one weird outlier item); omit below 3 matches rather than showing noise
4. Express endpoints + transport:
   - `GET /api/comparison/venues` — Google-backed list; ordinary limiter (~30/min/IP)
   - `GET /api/comparison/:placeId/stream` — **Server-Sent Events** (native `EventSource`; ~20 lines of Express, no new dependency; don't reuse Socket.IO — that's session-room machinery). Events: `storefront` (one per platform as its chain settles), `comparison` (final merged result incl. matched items + fetched_at), `error`. Fresh-Snapshot case: emit both `storefront` events + `comparison` immediately from the stored row and close — identical client path for cached and cold. Server writes the Snapshot once when both chains settle, **even if the client disconnects** (the spend already happened — keep the data). Rate limit ~5 cold compares/min/IP; streams served from a fresh Snapshot shouldn't count
   - Chain robustness: settle cap is **per actor run** (~300s, aligned with `timeout=280`), not per chain — the DoorDash chain is legitimately two serial runs (details, then menu) after the CSE query, so its worst honest case is ~10 min. A run exceeding its cap settles that platform as `status: 'failed'`, so the Snapshot always gets written and the skeleton never hangs forever. **Concurrent compares of the same Venue dedupe** through an in-memory in-flight map keyed by placeId — a second subscriber attaches to the running chain's stream instead of spawning new actor runs (AC3's "exactly one Snapshot"; fine on the single Railway instance — multi-instance would need a shared lock, note the ceiling with a `ponytail:` comment). `EventSource` auto-reconnects; on reconnect mid-flight the server re-emits already-settled `storefront` events from the in-flight state
...
Merged to main · Railway deploy green with `APIFY_TOKEN`, `GOOGLE_CSE_API_KEY`, and `GOOGLE_CSE_ID` set on the backend service · Apify account max-spend cap set · all CI tests green · the live E2E spec has passed · one genuine Compare performed on a phone against prod (own suburb, real Venue, both Platforms resolved, Snapshot row visible in Supabase).
...
4. Stale re-Compare of the DoorDash Storefront reuses the prior Snapshot's store URL — no CSE query, no details actor. (Uber Eats' search actor is simultaneously its resolution and menu fetch, so there's nothing to skip — unless step 0 validates the actor's `urls` input mode, in which case UE reuses its store URL too)
...
8. Per-IP rate limit on the compare endpoint → 429 with a friendly UI message
...
- **CI**: dummy `APIFY_TOKEN`/`GOOGLE_CSE_API_KEY`/`GOOGLE_CSE_ID` join the existing dummy-env block in `ci-cd.yml`; config reads them as optional strings (the `googlePlaces.apiKey` pattern). No network kill-switch — the deps seam means mocked tests never construct real URLs, and the Apify account spend cap is the money backstop.
...
- `APIFY_TOKEN` currently sits in the **repo-root** `.env` (a scratch file for API experiments, not app config). For the app: add it to `backend/.env` and to the Railway backend service, read via the existing `backend/src/config` pattern. `SUPABASE_SERVICE_ROLE_KEY` is already in `backend/.env`.
- The DD recipe adds a second credential pair: **`GOOGLE_CSE_API_KEY` + `GOOGLE_CSE_ID`** (Custom Search JSON API + Programmable Search Engine) — same treatment: `backend/.env` + Railway backend service, and both are part of the DoD's env check alongside `APIFY_TOKEN`.
...
- CSE is now load-bearing for half the comparison: its result quality is unproven until step 0, and its free quota (100 queries/day) is shared across all venues — quota exhaustion surfaces as DoorDash `failed` (2-min retry window applies), not as an outage
