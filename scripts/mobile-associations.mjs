...
export function associations({ APPLE_TEAM_ID, IOS_APP_ID, ANDROID_APP_ID, ANDROID_CERT_SHA256 }) {
  ...
  for (const id of [IOS_APP_ID, ANDROID_APP_ID]) {
    if (!/^[a-z][a-z0-9_]*(?:\.[a-z][a-z0-9_]*)+$/.test(id ?? '')) throw new Error('Provide both application IDs');
  }
  const fingerprints = (ANDROID_CERT_SHA256 ?? '').split(',').map((value) => value.trim().toUpperCase());
  ...
  return {
    ...
    'assetlinks.json': [{ relation: ['delegate_permission/common.handle_all_urls'], target: {
      namespace: 'android_app', package_name: ANDROID_APP_ID, sha256_cert_fingerprints: fingerprints,
    } }],
  };
}
