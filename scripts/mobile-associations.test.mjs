...
test('associations require real identity inputs and carry only the supported routes', () => {
  const input = { APPLE_TEAM_ID: 'TESTTEAM12', IOS_APP_ID: 'it.com.dinder.app.dev',
    ANDROID_APP_ID: 'it.com.dinder.app.dev', ANDROID_CERT_SHA256: Array(32).fill('AB').join(':') };
  ...
  assert.deepEqual(files['apple-app-site-association'].applinks.details[0].paths,
    ['/join', '/session/*', '/list/*', '/auth/callback']);
