...
const pkg = JSON.parse(
  readFileSync(
    new URL('../node_modules/capacitor-secure-storage-plugin/package.json', import.meta.url)
  )
);
if (pkg.version !== '0.13.0')
  throw new Error('Review the native storage patch for this dependency version');
const patch = 'patches/capacitor-secure-storage-plugin+0.13.0.patch';
const run = (args) =>
  spawnSync('git', ['apply', '--ignore-whitespace', ...args, patch], { cwd, encoding: 'utf8' });
