...
describe('parseComparisonSse', () => {
  ...
  it('rejects malformed and unknown events instead of passing a false smoke', () => {
    ...
    expect(() =>
      parseComparisonSse('event: error\ndata: {"code":"STREAM_CLOSED","message":"client only"}\n\n')
    ).toThrow('Invalid comparison SSE data');
  });
  ...
  it('requires a stale or missing Snapshot before exercising live credentials', () => {
    ...
    expect(() => assertColdSnapshot(snapshot, now)).toThrow('already has a fresh Snapshot');
    // ...and anything between the old 20-minute window and 6 hours is rejected too.
    expect(() =>
      assertColdSnapshot({ ...snapshot, fetchedAt: '2026-07-13T05:20:00.000Z' }, now)
    ).toThrow('already has a fresh Snapshot');
    // At exactly 6 hours the Snapshot is cold and live credentials may run.
    expect(() =>
      assertColdSnapshot({ ...snapshot, fetchedAt: '2026-07-13T02:20:00.000Z' }, now)
    ).not.toThrow();
  ...
  it('reads the latest run ID for every configured actor', async () => {
    ...
    await expect(
      latestActorRunIds(['uber/actor', 'door/actor'], 'token-123', fetchImpl as typeof fetch)
  ...
  it('fails when a fresh replay starts another actor run', () => {
    expect(() => assertActorRunIdsUnchanged(['ue-1', 'dd-1'], ['ue-1', 'dd-1'])).not.toThrow();
    expect(() => assertActorRunIdsUnchanged(['ue-1', 'dd-1'], ['ue-2', 'dd-1'])).toThrow(
      'Fresh replay started an unexpected actor run'
    );
  });
});
