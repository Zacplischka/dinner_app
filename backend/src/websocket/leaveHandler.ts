...
export async function handleSessionLeave(
  ...
  try {
    ...
    if (!validation.success) {
      console.warn(
        `Rejected session:leave for socket ${socket.id}: invalid payload - ${validation.error.errors[0].message}`
      );
      return callback({
        success: false,
    ...
    if (!participant) {
      console.warn(
        `Rejected session:leave for ${sessionCode}: socket ${socket.id} is not a participant`
      );
      return callback({
        success: false,
