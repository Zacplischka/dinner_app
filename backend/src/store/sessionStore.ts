...
export async function readSession(sessionCode: string): Promise<Session | null> {
...
export async function getSessionTtl(sessionCode: string): Promise<number> {
 ...
 */
export async function computeAndStoreResults(sessionCode: string): Promise<{
  ...
  const selectionKeys = participants.map((p) =>
 ...
 */
export async function resetForRestart(sessionCode: string): Promise<void> {
  const participantIds = await redis.smembers(participantsKey(sessionCode));
