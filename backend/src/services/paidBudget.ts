...
// day, spent before the paid call goes out. Per-IP windows slow one client
...
const BUDGET_TTL_MS = 48 * 3_600_000;
...
export const budgetKey = (sku: PaidSku) =>
  `budget:${sku}:${new Date().toLocaleDateString('en-CA', { timeZone: 'America/Los_Angeles' })}`;
...
const REFUSALS: Record<PaidSku, string> = {
  ...
  coldComparison:
    'New price comparisons are temporarily unavailable: the app has reached its daily limit. Please try again later.',
};

/**
 * Spends one call of a SKU's daily ceiling, or refuses with RATE_LIMITED so
 * the paid call never goes out. Fails closed, like the Spoonacular points
 * guard: a Redis error throws, and nothing is spent upstream.
 */
